...
class AnalyticsService {
    ...
    async getSalesSummary(range) {
        ...
        const currentPeriod = await prisma_1.prisma.order.aggregate({
            where: {
                createdAt: {
                    gte: range.startDate,
                    lte: range.endDate
        ...
        const previousPeriod = await prisma_1.prisma.order.aggregate({
            where: {
                createdAt: {
                    gte: previousStart,
                    lte: previousEnd
     ...
     */
    async getTopProducts(limit = 10, range) {
        const orderWhere = {
            paymentStatus: 'PAID',
            ...(range && {
                createdAt: {
        ...
        const revenueData = await prisma_1.prisma.orderItem.groupBy({
            by: ['productId'],
            where: {
                order: orderWhere,
     ...
     */
    async getPaymentBreakdown(range) {
        const whereClause = {
            order: {
                paymentStatus: 'PAID',
                ...(range && {
    ...
    async getLowStockItems() {
        ...
        const lowStock = await prisma_1.prisma.$queryRaw `
      SELECT id, name, unit, 
     ...
     */
    async getDailySales(range) {
        // Group by businessDate which is already a DATE type
        const dailySales = await prisma_1.prisma.order.groupBy({
            by: ['businessDate'],
            where: {
                businessDate: {
                    gte: range.startDate,
