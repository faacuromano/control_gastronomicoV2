...
exports.orderNumberService = exports.OrderNumberService = exports.OrderNumberGenerationError = void 0;
 ...
 */
class OrderNumberGenerationError extends Error {
    code;
    context;
...
exports.OrderNumberGenerationError = OrderNumberGenerationError;
/**
...
class OrderNumberService {
    ...
    async getNextOrderNumber(tx) {
        ...
        const operationId = (0, uuid_1.v4)(); // For correlating logs
        try {
            ...
            const sequenceKey = (0, businessDate_1.getBusinessDateKeyHourly)(new Date());
            ...
            const orderNumber = await this.getNextSequenceNumber(tx, sequenceKey, businessDate, operationId);
            ...
            logger_1.logger.error('ORDER_ID_GENERATION_FAILED', {
                operationId,
            ...
            throw new OrderNumberGenerationError('Failed to generate Order ID', 'ORDER_ID_GENERATION_FAILED', { originalError: error });
        }
    }
    /**
     ...
     */
    generateAndValidateUuid(operationId) {
        ...
        if (!(0, uuid_1.validate)(uuid)) {
            const error = new OrderNumberGenerationError(`UUID generation failed validation: ${uuid}`, 'UUID_VALIDATION_FAILED', { generatedUuid: uuid, operationId });
        ...
        if ((0, uuid_1.version)(uuid) !== 4) {
            const error = new OrderNumberGenerationError(`UUID version mismatch: expected v4, got v${(0, uuid_1.version)(uuid)}`, 'UUID_VERSION_MISMATCH', { generatedUuid: uuid, version: (0, uuid_1.version)(uuid), operationId });
            logger_1.logger.error('UUID_VERSION_MISMATCH', {
                operationId,
                generatedUuid: uuid,
                expectedVersion: 4,
                actualVersion: (0, uuid_1.version)(uuid),
                severity: 'HIGH'
            });
            throw error;
        }
     ...
     */
    async getNextSequenceNumber(tx, sequenceKey, businessDate, operationId) {
        let lastError = null;
        ...
        `;
                if (existing.length > 0) {
                    ...
                    if (!sequence || sequence.id === undefined || sequence.currentValue === undefined) {
                        throw new OrderNumberGenerationError('Sequence found but data is malformed', 'SEQUENCE_DATA_MALFORMED', { existing, operationId, attempt });
                    }
                    ...
                    if (newValue > this.MAX_ORDER_NUMBER) {
                        logger_1.logger.warn('ORDER_NUMBER_EXCEEDS_EXPECTED_RANGE', {
                            operationId,
                            sequenceKey,
                    ...
                    logger_1.logger.debug('SEQUENCE_INCREMENTED', {
                        operationId,
                        sequenceKey,
                ...
                if (!isRetryable) {
                    throw new OrderNumberGenerationError(`Non-retryable error during sequence generation: ${error.message}`, 'SEQUENCE_GENERATION_NON_RETRYABLE', { error, operationId, sequenceKey });
                }
        ...
        logger_1.logger.error('ORDER_SEQUENCE_GENERATION_EXHAUSTED_RETRIES', {
            operationId,
            sequenceKey,
            businessDate: businessDate.toISOString(),
        ...
        throw new OrderNumberGenerationError(`Failed to generate order number after ${this.MAX_RETRIES} attempts: ${lastError?.message}`, 'SEQUENCE_GENERATION_MAX_RETRIES_EXCEEDED', { lastError, operationId, sequenceKey, attempts: this.MAX_RETRIES });
    }
    /**
