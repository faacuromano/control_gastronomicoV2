...
/**
 * Order row (minimal fields needed for backfill)
 */
interface OrderRow {
  id: number;      // Legacy INT id (will become legacy_id after migration)
  uuid: string | null;
}
 ...
 */
async function verifyPreconditions(): Promise<void> {
  log('INFO', 'Verifying preconditions...');
  
  // CHECK 1: UUID column exists
  const columns = await prisma.$queryRaw<Array<{ column_count: bigint }>>`
  ...
  const nullability = await prisma.$queryRaw<Array<{ is_nullable: string }>>`
 ...
 */
async function countOrdersWithoutUuid(): Promise<number> {
 ...
 * Counts total orders
 */
async function countTotalOrders(): Promise<number> {
...
async function processBatch(
  ...
  for (const order of ordersToUpdate) {
    ...
    if (!uuidValidate(uuid)) {
      throw new Error(
        `CRITICAL: UUID generation failed validation for order ${order.id}: ${uuid}`
      );
    }
    ...
    if (uuidVersion(uuid) !== 4) {
      throw new Error(
        `CRITICAL: UUID version mismatch for order ${order.id}: expected v4, got v${uuidVersion(uuid)}`
      );
    }
  ...
  if (uniqueUuids.size !== uuidMap.size) {
    throw new Error(
      `CRITICAL: UUID collision detected in batch ${batchNumber}! ` +
...
async function verifyDataIntegrity(): Promise<void> {
  ...
  if (nullCountValue > 0) {
    throw new Error(
      `INTEGRITY FAILURE: ${nullCountValue} orders still have uuid = NULL`
    );
  }
  log('SUCCESS', 'No NULL UUIDs found');
  
  // CHECK 2: No duplicate UUIDs
  const duplicates = await prisma.$queryRaw<Array<{ uuid: string; dup_count: bigint }>>`
  ...
  if (duplicates.length > 0) {
    ...
    throw new Error(
      `INTEGRITY FAILURE: Found ${duplicates.length} duplicate UUIDs`
    );
  }
  log('SUCCESS', 'No duplicate UUIDs found');
  
  // CHECK 3: Verify UUID format (sample 1000 random rows)
  const invalidUuids = await prisma.$queryRaw<Array<{ id: number; uuid: string }>>`
    SELECT id, uuid 
    FROM \`Order\` 
    WHERE uuid IS NOT NULL
  ...
  if (invalidUuids.length > 0) {
    ...
    throw new Error(
      `INTEGRITY FAILURE: Found ${invalidUuids.length} invalid UUIDs`
    );
  }
